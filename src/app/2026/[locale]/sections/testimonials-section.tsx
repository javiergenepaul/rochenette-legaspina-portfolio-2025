  ...
  return (
    ...
    >
      ...
      <p className="text-woodsmoke-500 dark:text-woodsmoke-400 text-[0.95rem] leading-[1.75] max-w-[560px] mb-12">
        Feedback from colleagues, clients, and mentors I&apos;ve had the pleasure of working with.
      </p>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-5">
        {TESTIMONIALS.map(({ text, name, role, initials, avatarColor, stars }) => (
          <div
            key={name}
            className={cn(
          ...
          >
            {/* Top accent bar */}
            <div className="absolute top-0 left-0 right-0 h-[3px] bg-linear-to-r from-amethyst-500 to-amethyst-300" />
            ...
            <div className="flex gap-0.5">
              {Array.from({ length: stars }).map((_, i) => (
                <Star key={i} size={13} className="text-amethyst-500 fill-amethyst-500" strokeWidth={0} />
              ))}
            </div>
            ...
            <div className="flex items-center gap-3 mt-1">
              <div className={cn("w-11 h-11 rounded-full flex items-center justify-center shrink-0 text-white text-[0.8rem] font-poppins font-extrabold", avatarColor)}>
                {initials}
              </div>
              <div>
                ...
                <span className="flex items-center gap-1 text-[0.74rem] text-woodsmoke-400">
                  <Building2 size={11} className="text-amethyst-300" strokeWidth={1.75} />
                  {role}
                </span>
              </div>
            </div>
